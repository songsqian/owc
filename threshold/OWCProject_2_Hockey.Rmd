...
First, try the hockey stick model code without considering multilevel structure
```{r, include=F}
packages(rstan)
packages(rv)
...
stan.in0 <- function(infile=owc_daily, x="maTPLD05", y="TP_ef",
    ...
    y <- log(infile[,y])
##    gr <- as.numeric(ordered(infile[,3], levels=c("Spring", "Summer", "Autumn")))
    n <- dim(infile)[1]
##    R <- max(gr)
    inits <- list()
    if (stdz) theta <- 0.04
	...
	inits[[i]] <- list(beta0=rnorm(1), delta=runif(1),
			   phi=runif(1, range(x)[1], range(x)[2]),
                           ##re0=rep(0, R), re1=rep(0,R), reD=rep(0,R), reP=rep(0,R),
                           sigma=runif(1)
                           ##sigma0=runif(1),sigmaD=runif(1), sigmaP=runif(1)
                           )
    para <- c("beta0", "delta", "phi", 
	      ##"re0", "re1", "reD", "reP",
	      "sigma"#, "sigma0","sigma1","sigmaD","sigmaP"
              )
...
- Compiling the Stan model without running to create a model object
- Updating (MCMC runs)
...
## Plotting
  tikz(file="combinedFig.tex", width=4, height=2.25, standAlone = F)
  par(mar=c(3,3,1,0.5), mgp=c(1.5,0.1,0), las=1, tck=0.01)
  xx <- log(owc_daily$maTPLD05)
  yy <- log(owc_daily$TP_ef)
  plot(xx,yy,type="n", xlab="log TP loading", ylab="TP effluent concentration")
  for (i in 1:200){
      j <- sample(1:length(fit2coef$beta0),1, replace=F)
      curve(hockey_smooth(x, fit2coef$beta0[j], 0, #fit2coef$beta1[j],
                          fit2coef$delta[j],
                          fit2coef$phi[j],
                          input.to.bugs$data$theta), add=T, col="grey")
  }
  points(log(owc_daily$maTPLD05), log(owc_daily$TP_ef))
  curve(hockey_smooth(x, fit2coef_sum[1,7], 0, fit2coef_sum[2,7], fit2coef_sum[3,7],
...
## NADB plot
## nadb_path <- "C:/users/EESwithR/data"  
nadb_path <- "https://github.com/songsqian/eesR/tree/master/R/Data" 
...
## Multilevel Smoothed Hockey Stick Model
...
tikz(file="combinedFig.tex", height=5, width=6.5, standAlone = T)

par(oma=c(2, 2, 0.25,1), mar=c(1.5, 1.5, 1, 1), mgp=c(1.25,0.1,0), las=1, tck=0.01)
plot(xx,yy,type="n", xlab="", ylab="", axes=F)
  for (i in 1:200){
    j <- sample(1:length(fit2coefSN$B0),1, replace=F)
    curve(hockey_smooth(x, fit2coefSN$B0[j], 0,
##                           fit2coefSN$B1[j]+fit2coefSN$delB1.1[j], 
                           fit2coefSN$De[j],
                           fit2coefSN$Ph[j],
                           input.to.bugs$data$theta), add=T, col="grey")
  }
#  temp <- owc_daily$Quarter=="Q1"
  points(log(owc_daily$maTPLD05), log(owc_daily$TP_ef))
  curve(hockey_smooth(x, fit2coefSN_sum[1,2], 0, 
##                         fit2coefSN_sum[2,7] + fit2coefSN_sum[9,7], 
  ...
  axis(1)
  box()
  mtext("log TP loading", 1, 0.75,T)
  mtext("log TP concentration", 2, 0.75, T, las=0)
dev.off()

## using ggplot2

xrng <- range(df$x)
df2 <- data.frame(x = seq(xrng[1], xrng[2],,50),
                  y = hockey_smooth(seq(xrng[1], xrng[2],,50),
