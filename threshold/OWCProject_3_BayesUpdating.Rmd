...
The derived priors are used for model updating. For simplicity, we will assume model parameters are a priori independent of each other (an assumption that requires centering the predictor) and a
normal-inverse-gamma conjugate prior.  That is, for each parameter $\theta$ ($=\{ \beta_0, \delta,\phi\}$), we use 
$$
\begin{array}{rcl}
...
```
The updated output needs to be transformed back to its original scale. As a default, I fit the model using normalized loading, with the normalizing constants saved in the input data list.  After each
updating run, the resulting model parameters are converted back to their original scale:
...
fit <- stan_model(model_code = stan_model3)
fit4 <- sampling(fit=fit, data = input.to.stan$data, 
                 init=input.to.stan$inits,
                 pars = input.to.stan$para,
                 iter=niters, thin=nthin, chains=input.to.stan$n.chains,
                 control = list(adapt_delta = 0.99, max_treedepth=20))
...
fit <- stan_model(model_code = stan_model3)
fit6 <- sampling(fit=fit, data = input.to.stan$data, init=input.to.stan$inits,
	    pars = input.to.stan$para,
	    iter=niters, thin=nthin, chains=input.to.stan$n.chains,
	    control = list(adapt_delta = 0.99, max_treedepth=20))
...
fit <- stan(model_code = stan_model3,
	    data = input.to.stan$data, init=input.to.stan$inits,
	    pars = input.to.stan$para,
	    iter=5, chains=input.to.stan$n.chains)
fitCC2012 <- stan(fit=fit, data = input.to.stan$data, input.to.stan$inits,
                  pars = input.to.stan$para,
                  iter=niters, thin=nthin, chains=input.to.stan$n.chains,
                  control = list(adapt_delta = 0.99, max_treedepth=20))
...
fitcoefCC12 <- rvsims(as.matrix(as.data.frame(extract(fitCC2012, permute=T))))
...
fit <- stan(model_code = stan_model3,
	    ...
	    pars = input.to.stan$para,
	    iter=5, chains=input.to.stan$n.chains)
fitCC2013 <- stan(fit=fit, data = input.to.stan$data, input.to.stan$inits,
                  pars = input.to.stan$para,
                  iter=niters, thin=nthin, chains=input.to.stan$n.chains,
                  control = list(adapt_delta = 0.99, max_treedepth=20))
...
fitcoefCC13 <- rvsims(as.matrix(as.data.frame(extract(fitCC2013, permute=T))))
...
fit <- stan(model_code = stan_model3,
	    ...
	    pars = input.to.stan$para,
	    iter=5, chains=input.to.stan$n.chains)
fitCC2014 <- stan(fit=fit, data = input.to.stan$data, input.to.stan$inits,
                  pars = input.to.stan$para,
                  iter=niters, thin=nthin, chains=input.to.stan$n.chains,
                  control = list(adapt_delta = 0.99, max_treedepth=20))
...
tempFit2<- rvsims(as.matrix(as.data.frame(extract(fit2, permute=T))))
fitcoefCC14 <- rvsims(as.matrix(as.data.frame(extract(fitCC2014, permute=T))))
...
fitcoefOlt1 <- rvsims(as.matrix(as.data.frame(extract(fitOlt1, permute=T))))
...
fit <- stan_model(model_code = stan_model3)
fitOlt2 <- stan(fit=fit, data = input.to.stan$data, ##input.to.stan$inits,
                  pars = input.to.stan$para,
                  iter=niters, thin=nthin, chains=input.to.stan$n.chains,
                  control = list(adapt_delta = 0.99, max_treedepth=20))

fitcoefOlt2 <- rvsims(as.matrix(as.data.frame(extract(fitOlt2, permute=T))))
...
png(file="olt_post.png", height=3*120, width=5*120)
p + facet_wrap(~Wetland, nrow = 1)
dev.off()
...
## using ggplot
seasonal$name <- c("OWC", "OWC_sp", "OWC_sm", "OWC_fl", "OWC_wn", "CC_12", "CC_13", "CC_14", "CWC", "CWC_update", "Olt1", "Olt2")
d <- seasonal[-(2:5),]
