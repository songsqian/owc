...
This file includes data processing code for the Old Woman Creek project. Data used include daily monitoring data from Old Woman Creek (divided into four seasons), three year intensive monitoring at Crane Creek, two phases of monitoring of Coldwater Creek wetland, and a long term data from Olentangy River Wetlands.  The R code organizes the input data into two main columns: input P loading and output P concentration, using necessary grouping variables (season, location, year, etc.).  
...
Data files include water quality data from two monitoring sites (input at Berlin Road and output at site "OL"), USGS flow data, and a separate file on the OWC mouth status (open or closed).
...
## Crane Creek Data
Crane Creek data are measured every 15 minutes.  The site has only one opening point to Lake Erie.  Water inflow and outflow are controlled by Lake.  
...
```
 Summary
 - Crane Creek data are relatively simple in their structure.  The input is aggregated to weekly or monthly means.
 - 
...
cwc_temp$TPLoading <- cwc$TP_in*cwc$flow * 0.893 ## tons per year
cwc_temp$DRPLoading <- cwc$DRP_in*cwc$flow * 0.893  ## tons per year
cwc_temp$NO3Loading <- cwc$NO3_in*cwc$flow * 0.893  ## tons per year
...
## New data were made available near the end of the project
...
cwc_tp <- read.csv(paste(dataDIR, "CWC_TP.csv", sep="/"))[1:204,1:4]
...
cwc_tp_in <- cwc_tp %>% filter(Site==levels(Site)[2])
cwc_tp_out <- cwc_tp %>% filter(Site==levels(Site)[3])
...
cwc_monthly$TPload <- cwc_monthly$fmean*cwc_monthly$TP_in * 0.893
## tons per year

png(filename="cwc_new.png", width=4.75*120, height=3*120)
ggplot(cwc_monthly, aes(x=TPload, y=TP_out))+geom_point()+scale_x_log10()+scale_y_log10()+xlab("TP Loading (ton/yr)")+ylab("TP Concentration (mg/L)")
dev.off()
```
